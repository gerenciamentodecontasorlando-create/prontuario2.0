...
(() => {
  ...
  function renderSettings(){
    const wrap = document.createElement("div");
    const c = card("Configurações", "dados do profissional e trava por PIN");
    c.innerHTML += `
      <div class="grid2">
        <div class="card">
          <div class="cardHeader">
    ...
    $("#saveSettings", c).onclick = async () => {
      ...
      const b = $("#sPin2", c).value.trim();
      if (!a || a.length < 3) { toast("PIN muito curto."); return; }
      if (a !== b){ toast("PINs não conferem."); return; }
      state.settings.accessPin = a;
      await dbApi.put("settings", { key:"app_settings", value: state.settings });
      toast("PIN atualizado ✅");
    };

    $("#wipeAll", c).onclick = async () => {
    ...
    $("#exportBackup", c).onclick = async () => exportBackup();
    $("#importBackup", c).onclick = async () => importBackup();
  ...
  function openPrint(html, filename="documento.html"){
    ...
    $("#doPrint").onclick = () => {
      try{
        ...
        toast("Não deu pra acionar o print automático. Tente pelo menu do navegador.");
      }
    };
